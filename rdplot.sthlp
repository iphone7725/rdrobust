...
{viewerjumpto "Saved results" "rdplot##saved_results"}{...}
...
{p 4 8}{cmd:covs(}{it:covars}{cmd:)} specifies additional covariates used to construct the local-polynomial estimator(s).{p_end}
...
{p 4 8}{cmd:support(}{it:# #}{cmd:)} specifies an optional extended support of the running variable to be used in the construction of the bins. Default is the sample range.

{p 4 8}{cmd:hide} omits the RD plot.

{p 4 8}{cmd:ci(}{it:cilevel}{cmd:)} optional graphical option to display confidence intervals of level {it:ci_level} for each bin. 

{p 4 8}{cmd:shade} optional graphical option to replace confidence intervals with shaded areas.
...
{p 4 8}{cmd:graph_options(}{it:gphopts}{cmd:)} specifies graphical options to be passed on to the underlying graph command.
...
{p 4 8}{cmd:rdplot} saves the following in {cmd:e()}:
